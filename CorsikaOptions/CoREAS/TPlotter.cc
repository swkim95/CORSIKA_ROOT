...
TPlotter::InitializeRadioSimulation()
{
  ...
  // if GDAS-based, the fRefractivityTable-vector has been pre-filled at this stage, but needs to be extrapolated to 112.8292 km
  double  minimumHeight;
  double maximumHeight = 112.82920 * km; // ToDo: get value from CORSIKA
  ...
  if (fRefractivityTable.size() == 0)
  {
    ...
    size_t numberOfPoints = (maximumHeight-minimumHeight) / fDistanceIncrement + 1;
  ...
  {
    // use refractivity table pre-filled from GDAS data, extrapolating to maximumHeight

    if (IsMainThread())
      cout << "Using refractivity lookup table from GDAS-based file.\n";

    // sanitize refractivity table (clip possible illegal trailing entries)
    while (fHeightTable.at(fHeightTable.size()-1) < fHeightTable.at(fHeightTable.size()-2))
    {
      fHeightTable.pop_back();
      fRefractivityTable.pop_back();      
    }

    // check consistency of height and refractivity tables
    if (fHeightTable.size() != fRefractivityTable.size())
    {
      if (IsMainThread()) {
        cout << "\n\n"
             << " ###################################\n"
             << " Error, length of GDAS-based fHeightTable is different from length of GDAS-based fRefractivityTable. This should not happen, please report this issue!\n"
             << " ###################################\n\n"
             << endl;
      }
      exit (11);   
    ...
    double gdasHeight = fHeightTable.back();
    fDistanceIncrement = (gdasHeight - minimumHeight) / (fHeightTable.size()-1);

    if (gdasHeight < maximumHeight)
    {
      if (IsMainThread()) {
        cout << "GDAS-provided refractivity table extends to altitude of " << gdasHeight / km << " km.\n"
             << "Extrapolating to edge of atmosphere at " << maximumHeight / km << " km using Gladstone-Dale law.\n"
             << endl;
      }
    }
    ...
    // extrapolate to edge of atmosphere using Gladstone-Dale-Law
    long numExtraPoints = (maximumHeight - gdasHeight) / fDistanceIncrement;
    fHeightTable.reserve(fHeightTable.size()+numExtraPoints);
    fRefractivityTable.reserve(fRefractivityTable.size()+numExtraPoints);
    double rho_gdasmax = rhof_(gdasHeight);
    double N_gdasmax = fRefractivityTable.back();
    for (long i=1; i<=numExtraPoints; ++i)
    {
      double h = gdasHeight+i*fDistanceIncrement;
      double N = rhof_(h)/rho_gdasmax * N_gdasmax;
      fHeightTable.push_back(h);
      fRefractivityTable.push_back(N);
    }
  ...
  if (IsMainThread())
  {
    cout << " Tabulated atmosphere inside CoREAS has following parameters" << std::endl
         ...
         << "   Number of points: " << fHeightTable.size() << "\n" << std::endl;
  }
  ...
  if ((fCurved) && (fZenith>=fTabulationThresholdZenithAngle))
  {
    ...
    // reserve memory for tanThetaTables --- should be refactored to one table per unique antenna altitude rather than one table per antenna
...
{
  fHeightTable.reserve(nPoints-1);
  fRefractivityTable.reserve(nPoints-1);
  for (int i=0; i<nPoints-1; i++) // for some reason, the last entry coming from CORSIKA has zeroes inside - so skip it
  {
