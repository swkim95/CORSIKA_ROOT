...
void TC2R::Write(const crs::TSubBlock &SubBlock) {
  ...
  if(fFile) {
    ...
    case crs::TSubBlock::eEVTH:
      ...
      fCurrentShower->Init(); // HERE initiate shower per every new event
      fCurrentShower->AddEventHeader(SubBlock);      
      break;

    case crs::TSubBlock::ePARTDATA:
      fCurrentShower->AddParticle(SubBlock); // HERE Add particle info to each shower
      AddParticleBlock(SubBlock);
      break;

    case crs::TSubBlock::eLONG:
      fCurrentShower->AddLongitudinal(SubBlock); // HERE add longitudinal info to each shower
    ...
    case crs::TSubBlock::eEVTE:
      ...
      fCurrentShower->End(); // HERE Clear shower when event ends
...
void TC2R::AddParticleBlock(const crs::MParticleBlock &Data) {
  ...
  for(iEntry = Data.ParticlesBegin(); iEntry != Data.ParticlesEnd(); ++iEntry) {
    ...
    switch(iEntry->GetType()) {
      ...
      case crs::eParticle:
      case crs::eNucleus:
        new((*fParticles) [fNParticles++]) crsIO::TParticle(*iEntry);
        break;

      case crs::eCherenkov:
        new((*fCherenkov) [fNCherenkov++]) crsIO::TCherenkov(*iEntry);
        break;
